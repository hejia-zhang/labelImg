...
def add(add_table, new_labels, label_content):
    ...
    labels.sort(key=lambda label: yolov3_to_minmax(
        [label.split(' ')[1], label.split(' ')[2], label.split(' ')[3], label.split(' ')[4]])[0] +
                                  yolov3_to_minmax(
                                      [label.split(' ')[1], label.split(' ')[2], label.split(' ')[3],
                                       label.split(' ')[4]])[2])
...
if __name__ == '__main__':
    add_table_lst = [
        {'container_type': '5', 'range': range(3502, 3686), 'contained_type': ['16']}]
    run(add_table_lst,

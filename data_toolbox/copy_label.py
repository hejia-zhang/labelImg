...
def run(copy_table_lst, label_dir, label_out_dir):
    ...
    for label_file in labels:
        ...
        with open(os.path.join(label_dir, label_file), 'r') as f:
            ...
            for copy_table in copy_table_lst:
                if int(label_file.split('.')[0]) in copy_table['range']:
                    new_labels.append(copy_table['label'])
...
if __name__ == '__main__':
    copy_table_lst = [{'label': '27 0.216797 0.578472 0.094531 0.073611', 'range': range(5985, 4957)}]
    run(copy_table_lst,
